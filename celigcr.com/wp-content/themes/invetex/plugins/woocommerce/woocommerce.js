...
jQuery( document ).on( 'action.ready_invetex', function() {
	...
	$document.on( 'action.resize_invetex', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.invetex_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		invetex_set_cookie( 'invetex_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	invetex_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function invetex_woocommerce_add_inc_dec() {
		jQuery( '.woocommerce div.quantity:not(.inited),.woocommerce-page div.quantity:not(.inited)' ).each( function() {
			...
			$self
				...
				.on( 'click', '>span', function(e) {
					invetex_woocomerce_inc_dec_click( jQuery( this ) );
					e.preventDefault();
					return false;
				} );
		} );
	...
	function invetex_woocomerce_inc_dec_click(button) {
			...
			.on( 'click', function(e) {
				...
				jQuery.get(	link ).done(
					function(response) {
						...
						invetex_import_inline_styles( response );
						// Get tags 'link' from response and add its to the 'head'
						invetex_import_tags_link( response );
						...
						if ( posts_container.length > 0 ) {
							...
							INVETEX_STORAGE['init_all_mediaelements'] = true;
							$document
	...
	$document.on( 'action.scroll_invetex', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < invetex_window_scroll_top() + invetex_window_height() * 1.5 ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		invetex_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					invetex_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					invetex_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		invetex_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			invetex_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function invetex_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = invetex_window_width() + ( invetex_window_height() < $document.height() || invetex_window_scroll_top() > 0 ? 16 : 0 );
				}

				var tbl = table_cart;

				if ( w < INVETEX_STORAGE['mobile_layout_width'] ) {
				...
				if ( w >= INVETEX_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
